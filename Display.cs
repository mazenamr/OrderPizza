...
namespace OrderPizza
{
    class Display
    {
        ...
        public string EditComponent(IEnumerable<(string name, double price)> available, string stringName, string intName)
        {
            Table orderTable = new Table()
                .Border(TableBorder.Rounded)
                .Title($"[underline]Edit {stringName}[/]")
                .AddColumn(new TableColumn(stringName))
                .AddColumn(new TableColumn(intName).Centered())
                .Expand();
        ...
        public List<string> GetNames(IEnumerable<string> names)
        {
            MultiSelectionPrompt<string> prompt = new MultiSelectionPrompt<string>()
                .AddChoices(names)
                .NotRequired();
        ...
        public double GetNumber(string stringName, string intName) {
            AnsiConsole.Clear();
            return AnsiConsole.Prompt(
                new TextPrompt<double>($"Enter a {intName} for the {stringName}")
                    .Validate(num => num <= 0 ?
